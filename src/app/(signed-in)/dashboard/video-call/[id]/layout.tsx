...
export default function Layout({
	children,
}: LayoutProps<"/dashboard/video-call/[id]">) {
	const { user } = useUser();
	const { id } = useParams();
		...
		<StatusCard
			...
			action={
				<button
					...
					className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
				>
					Retry
				</button>
			}
